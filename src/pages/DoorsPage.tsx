...
import { DOORS_SUBPAGES } from '../data/pagesContent';

interface DoorsPageProps {
  onOpenQuoteModal: (productTitle?: string) => void;
}

export const DoorsPage: React.FC<DoorsPageProps> = ({ onOpenQuoteModal }) => {
    ...
    <div className="pt-36 pb-24 bg-[#fafaf9]">
      <div className="site-container">
        ...
        <div className="text-[11px] font-mono uppercase tracking-widest text-[#8e9199] mb-8">
          <Link to="/" className="hover:text-[#111215]">Pradžia</Link> / <span>Durys</span>
        </div>
        ...
        <div className="max-w-3xl space-y-4 mb-16">
          <div className="eyebrow">
            ...
            <span>LAUKO, BALKONO, TERASINĖS IR ŠARVUOTOS DURYS</span>
          </div>
          <h1 className="text-4xl sm:text-6xl font-light tracking-tight text-[#111215] leading-[1.08]">
            Durys namams ir butams.{' '}
            <span className="font-serif italic font-normal text-[#c81e1e]">Saugumas ir šiluma</span>
          </h1>
          <p className="text-base text-[#64676f] leading-relaxed">
            Plastikinės balkono ir lauko durys, panoraminės aliuminio HST terasinės sistemos bei patikimos šarvuotos durys butams ir namams su termo tiltelio nutraukimu.
          </p>
        </div>

        {/* Product Cards Grid with Original Photos */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {DOORS_SUBPAGES.map((item, idx) => {
              ...
              >
                <div>
                  ...
                  <div className="aspect-[4/3] rounded-lg overflow-hidden bg-[#ebebeb] mb-6 relative group/img">
                    {item.imageUrl.startsWith('http') || item.id.includes('terasines') || item.id.includes('vidaus') ? (
                      <img
                        src={item.imageUrl}
                        alt={item.title}
